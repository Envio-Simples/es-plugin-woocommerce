...
class Es_Plugin_Woo_Label
{
    ...
    public function enviosimples_order_processing($order_id)
    {
        ...
        $label_data = [
            ...
            'destinatario' => [
                ...
                'cnpjCpf' => $this->only_numbers($destinatario_cnpjCpf),
                ...
                'uf' => $destinatario_uf,
                'cep' => $this->only_numbers($destinatario_cep),
                'celular' => $this->only_numbers($destinatario_celular)
            ],
            'volume' => $volumes,
        ...
        if (count($meta_group) > 0) {
            foreach ($meta_group as $meta) {
                ...
                if ($meta->key == "_billing_cpf") $cpf = $this->only_numbers($meta->value);
                if ($meta->key == "_billing_cnpj") $cnpj = $this->only_numbers($meta->value);
                if ($meta->key == "_billing_number") $billing_number = $meta->value;
                if ($meta->key == "_billing_neighborhood") $billing_neighborhood = $meta->value;
                if ($meta->key == "_billing_cellphone") $billing_cellphone = $this->only_numbers($meta->value);
        ...
        $enviosimples = new Es_Plugin_Woo_API($token, $sandbox);
